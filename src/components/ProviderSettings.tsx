...
const providerStatusLabels: Record<HarnessProvider['status'], string> = {
  get ready() { return t("已连接") },
  get missing() { return t("组件缺失") },
  get disabled() { return t("已停用") },
...
function BailianProviderPanel({
  ...
  useEffect(() => {
    ...
    void getBailianProviderSettings()
      ...
      .catch((error) =>
        onAction(
          t("无法读取百炼配置：{0}", [error instanceof Error ? error.message : String(error)]),
        ),
      )
  ...
  const save = async () => {
    ...
    try {
      ...
      onAction(t("百炼 AK 已保存"))
    } catch (error) {
      onAction(
        t("保存失败：{0}", [error instanceof Error ? error.message : String(error)]),
      )
    } finally {
      setSaving(false)
    }
    ...
    <div className="api-provider-workspace">
      <section className="api-provider-form">
        <div className="api-provider-heading">
          ...
          <div>
            ...
            <h2>{t("阿里云百炼")}</h2>
            <p>{t("保存访问凭据后，可在扩展中按需添加在线模型。")}</p>
          </div>
          <span
            className={`provider-health ${settings.status === 'ready' ? 'ready' : ''}`}
          >
            <i />
            {settings.status === 'ready' ? t("AK 已配置") : t("待配置 AK")}
          </span>
        </div>
      ...
      <aside className="api-provider-summary">
        ...
        <p className="provider-summary-copy">
          {t("凭据只用于连接服务。需要使用的模型请在扩展中单独添加。")}</p>
        ...
        <div className="provider-safety-note">
          <ShieldCheck size={16} />
          <span>
            <strong>{t("显式云端执行")}</strong>
            <small>{t("只有选择百炼模型时，输入才会发送到百炼 API。")}</small>
          </span>
        </div>
      </aside>
    </div>
...
function CustomProviderPanel({
  ...
  useEffect(() => {
    ...
    void getApiProviderSettings()
      ...
      .catch((error) =>
        onAction(
          t("无法读取自定义 Provider：{0}", [error instanceof Error ? error.message : String(error)]),
        ),
      )
  ...
  const save = async () => {
    ...
    if (!settings.name.trim()) {
      onAction(t("请填写 Provider 名称"))
      return
    }
    if (!settings.baseUrl.trim()) {
      onAction(t("请填写 API Base URL"))
      return
    }
    if (!settings.llmEnabled && !settings.asrEnabled && !settings.ttsEnabled) {
      onAction(t("请至少启用一种 Provider 能力"))
      return
    }
    if (
      ...
      onAction(t("远程 Provider 需要填写 API Key"))
      return
    }
    setSaving(true)
    try {
  ...
  const removeProvider = async () => {
    ...
    if (
      !window.confirm(
        t("删除 Provider“{0}”？绑定它的自定义模型将不再显示。", [settings.name]),
      )
    ) {
      return
    }
    try {
      ...
      onCatalogChanged(await getHarnessCatalog())
      onAction(t("自定义 Provider 已删除；引用它的模型将不再显示"))
    } catch (error) {
      onAction(
  ...
  return (
    <div className="api-provider-workspace">
      <section className="api-provider-form">
        <div className="api-provider-heading">
          ...
          <div>
            ...
            <h2>{t("自定义 Provider")}</h2>
            <p>{t("连接、鉴权和能力协议在这里配置；模型只保存 Model ID。")}</p>
          </div>
          <span
            className={`provider-health ${settings.status === 'ready' ? 'ready' : ''}`}
          >
            <i />
            {settings.status === 'ready' ? t("已连接") : t("待配置")}
          </span>
        </div>

        <div className="provider-picker-row">
          <label>
            <span>{t("已保存的 Provider")}</span>
            <select
              value={settings.id}
              onChange={(event) => {
            ...
            >
              ...
              {settings.id.startsWith('api.new.') && (
                <option value={settings.id}>{t("新 Provider")}</option>
              )}
            </select>
          </label>
          <button className="secondary-action" type="button" onClick={addProvider}>
            <CirclePlus size={15} />{t("新增")}</button>
          <button className="icon-button danger" type="button" aria-label={t("删除 Provider")} onClick={() => void removeProvider()}>
            <Trash2 size={16} />
          </button>
        </div>
        ...
        <div className="provider-form-grid">
          <label>
            ...
            <input
              value={settings.id === "api.bailian" ? t("阿里云百炼") : settings.name}
              placeholder={t("例如团队网关或本地服务")}
          ...
          <label>
            ...
            <select value={settings.authType} onChange={(event) => update('authType', event.target.value as ApiProviderSettings['authType'])}>
              ...
              <option value="custom-header">{t("自定义 Header")}</option>
          ...
          <label className="provider-key-field">
            ...
            <input
              ...
              placeholder={
                settings.apiKeyConfigured
                  ? t("API Key 已保存 · 留空保持不变")
                  : t("本机 localhost 服务可留空")
              }
              onChange={(event) => setApiKey(event.target.value)}
            />
          </label>
          <label className="provider-key-field">
            <span>{t("附加请求 Header")}</span>
            <textarea
            ...
            />
            <small>{t("每行一个 Name: Value；支持 ")}{'{model}'}、{'{voice}'}、{'{uuid}'} {t(" 变量")}</small>
          </label>
        </div>
        ...
        <div className="provider-protocol-sections">
          ...
          {settings.asrEnabled && (
            <details><summary>{t("ASR 接口")}</summary><div className="provider-form-grid">
              <label><span>{t("请求格式")}</span><select value={settings.asrMode} onChange={(event) => update('asrMode', event.target.value as ApiProviderSettings['asrMode'])}><option value="multipart">{t("Multipart 文件上传")}</option><option value="binary">{t("原始音频 Body")}</option><option value="template-json-base64">{t("JSON 模板（Base64 音频）")}</option></select></label>
              <label><span>{t("接口路径")}</span><input value={settings.asrPath} placeholder="/audio/transcriptions" onChange={(event) => update('asrPath', event.target.value)} /></label>
              {settings.asrMode === 'multipart' && <><label><span>{t("模型字段")}</span><input value={settings.asrModelField} onChange={(event) => update('asrModelField', event.target.value)} /></label><label><span>{t("语言字段")}</span><input value={settings.asrLanguageField} onChange={(event) => update('asrLanguageField', event.target.value)} /></label><label><span>{t("提示词字段")}</span><input value={settings.asrPromptField} onChange={(event) => update('asrPromptField', event.target.value)} /></label></>}
              {settings.asrMode === 'template-json-base64' && <label className="provider-key-field"><span>{t("请求 JSON 模板")}</span><textarea value={settings.asrBodyTemplate} onChange={(event) => update('asrBodyTemplate', event.target.value)} /><small>{t("支持 ")}{'{audioBase64}'}、{'{model}'}、{'{language}'}、{'{prompt}'}、{'{uuid}'}</small></label>}
              <label><span>{t("文本响应路径")}</span><input value={settings.asrTextPointer} placeholder="/text" onChange={(event) => update('asrTextPointer', event.target.value)} /><small>{t("使用 JSON Pointer，例如 /channel/alternatives/0/transcript")}</small></label>
            </div></details>
          )}
          {settings.ttsEnabled && (
            <details><summary>{t("TTS 接口")}</summary><div className="provider-form-grid">
              <label><span>{t("请求格式")}</span><select value={settings.ttsMode} onChange={(event) => update('ttsMode', event.target.value as ApiProviderSettings['ttsMode'])}><option value="standard-json">{t("标准 JSON（model/input/voice）")}</option><option value="voice-path-json">{t("音色在 URL（text/model_id）")}</option><option value="nested-voice-json">{t("嵌套音色与音频设置")}</option><option value="query-model-json">{t("模型在 URL 或查询参数")}</option><option value="template-json">{t("自定义 JSON 模板")}</option></select></label>
              <label><span>{t("接口路径")}</span><input value={settings.ttsPath} placeholder="/audio/speech" onChange={(event) => update('ttsPath', event.target.value)} /><small>{t("可使用 ")}{'{model}'}、{'{voice}'} {t(" 和 ")}{'{speed}'} {t(" 变量")}</small></label>
              {settings.ttsMode === 'template-json' && <label className="provider-key-field"><span>{t("请求 JSON 模板")}</span><textarea value={settings.ttsBodyTemplate} onChange={(event) => update('ttsBodyTemplate', event.target.value)} /><small>{t("支持 ")}{'{text}'}、{'{model}'}、{'{voice}'}、{'{speed}'}、{'{speechRate}'}、{'{sampleRate}'}、{'{audioFormat}'}、{'{uuid}'}</small></label>}
              <label><span>{t("响应编码")}</span><select value={settings.ttsResponseEncoding} onChange={(event) => update('ttsResponseEncoding', event.target.value as ApiProviderSettings['ttsResponseEncoding'])}><option value="raw">{t("原始音频")}</option><option value="hex">{t("JSON 中的 Hex")}</option><option value="base64">{t("JSON 中的 Base64")}</option><option value="stream-base64">{t("分块 JSON / SSE Base64")}</option></select></label>
              {settings.ttsResponseEncoding !== 'raw' && <label><span>{t("音频响应路径")}</span><input value={settings.ttsAudioPointer} placeholder="/data/audio" onChange={(event) => update('ttsAudioPointer', event.target.value)} /></label>}
              <label><span>{t("音频格式")}</span><select value={settings.ttsAudioFormat} onChange={(event) => update('ttsAudioFormat', event.target.value as ApiProviderSettings['ttsAudioFormat'])}><option value="wav">WAV</option><option value="pcm16">{t("PCM16 单声道")}</option></select></label>
              <label><span>{t("采样率")}</span><input type="number" min="8000" max="96000" value={settings.ttsSampleRate} onChange={(event) => update('ttsSampleRate', Number(event.target.value))} /></label>
            </div></details>
          )}
        </div>
      ...
      <aside className="api-provider-summary">
        ...
        <h2>{t("连接信息")}</h2>
        <p className="provider-summary-copy">
          {t("Provider 只负责连接；模型名称与 Model ID 在扩展中管理。")}</p>
        <dl className="provider-contract-facts">
          <div>
            <dt>{t("状态")}</dt>
            <dd>{providerStatusLabels[provider?.status ?? settings.status]}</dd>
          ...
          <div>
            <dt>{t("接口")}</dt>
            <dd>{[settings.llmEnabled && 'LLM', settings.asrEnabled && 'ASR', settings.ttsEnabled && 'TTS'].filter(Boolean).join(' · ') || t("未启用")}</dd>
          </div>
          <div>
            <dt>{t("本地 API")}</dt>
            <dd>{runtime.apiUrl}</dd>
        ...
        <div className="provider-safety-note">
          ...
          <span>
            <strong>{t("显式云端执行")}</strong>
            <small>{t("只有使用其模型时，消息才会发送到配置的地址。")}</small>
          </span>
        </div>
      </aside>
    </div>
...
export function ProviderSettings({
  ...
  return (
    <div className="provider-settings">
      <div className="cloud-provider-tabs" aria-label={t("选择 Provider")}>
        <button
        ...
        >
          <Sparkles size={17} />
          <span>
            <strong>{t("阿里云百炼")}</strong>
            <small>{t("Qwen、FunASR 与 CosyVoice")}</small>
          </span>
        ...
        >
          ...
          <span>
            <strong>{t("自定义")}</strong>
            <small>{t("通用 LLM、ASR 与 TTS API")}</small>
          </span>
