...
import { openAgentUi } from '../services/harness'

export function PythonAgentWorkspace({ id, title }: { id: string; title: string }) {
  ...
  useEffect(() => {
    if (session && frameRef.current) {
      return attachAgentAudioBridge({ id: id, session, frame: frameRef.current })
    }
    return undefined
  }, [id, session])
  ...
  useEffect(() => {
    ...
    setSession(null); setError(''); setPageReady(false); setElapsed(0)
    setStages([{ message: '检查账号与启动条件', elapsedMs: 0 }])
    const start = async () => {
      const agentId = agentInstallRegistry.resolve(id).id
      unlisten = await listen<{ id: string; message: string; elapsedMs: number }>('agent-ui-progress', event => {
        if (!canceled && event.payload.id === agentId) setStages(current => [...current.slice(-19), { message: event.payload.message, elapsedMs: Date.now() - started }])
      })
      if (canceled) { unlisten(); return }
      if (canceled) return
      const value = await openAgentUi(agentId)
  ...
  return <section className="agent-browser" aria-label="本地 Agent">
    <div className="agent-browser-content">
      <>{session && <iframe ref={frameRef} src={session.url} title={session.title} allow="microphone" sandbox="allow-scripts allow-same-origin allow-forms allow-downloads" referrerPolicy="no-referrer" onLoad={() => { frameRef.current?.contentWindow?.postMessage({ type: 'toolkits-host-ready' }, new URL(session.url).origin); clearInterval(timerRef.current); setPageReady(true) }} />}
